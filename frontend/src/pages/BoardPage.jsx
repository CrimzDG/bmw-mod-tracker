...
export default function BoardPage() {
  ...
  function findStatusOfMod(id) {
    return mods.find(m => m.id === id)?.status
  }
  ...
  async function handleDragEnd({ active, over }) {
    ...
    // Dropped over a column (status string) or another mod card
    const overId = over.id
    const targetStatus = STATUSES.includes(overId)
      ? overId
      : findStatusOfMod(overId)

    if (!targetStatus) return

    if (mod.status !== targetStatus) {
